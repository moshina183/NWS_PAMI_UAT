package com.vinsys.diam.interceptor;
...
import com.vinsys.diam.esb.Helper;
import com.vinsys.diam.esb.IConstants;
import com.vinsys.diam.filters.AuthenticationFilter;
...
public class BaseInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		logger.info("Inside prehandle without validation");
		return true;
	...
	public boolean preHandle1(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (request.getHeader("Content-Type")==null || request.getHeader("Content-Type").contains("application/json")) {
			if (s.contains("/api/v1")) {
				...
				System.out.println(esbServiceCode);
				GetDetails getDetails = new GetDetails();
			...
			/**
			 * this is to check the request is v2
			 * 
			 */
			else {
				auditLogs = new TXNAuditLogs();
